...
TEXT_LIMIT = 180_000
EXCLUDED = {".git", ".klap", "graphify-out", "node_modules", "vendor", ".venv", "venv", ".pytest_cache", ".test-venv", "build", "dist"}
...
def _source_files(root: Path, suffixes: set[str]):
    """Walk source files without first descending into dependency trees."""
...
def entry_points(root: Path) -> tuple[list[dict], list[dict]]:
    ...
    for path in _source_files(root, {".py"}):
        if path.name == "system.py": continue
    ...
    for path in _source_files(root, {".js"}):
...
def external_systems(root: Path) -> tuple[list[dict], list[dict]]:
    ...
    candidates = list(_source_files(root, {".json", ".toml", ".yml", ".yaml", ".php", ".py", ".js", ".ts", ".env"}))
    for path in candidates[:800]:
        if "tests" in path.relative_to(root).parts or "frameworks" in path.relative_to(root).parts or path.name in {"system.py", "portal.py", "pyproject.toml"}: continue
        text=_read(path); rel=str(path.relative_to(root))
...
def deployment(root: Path) -> tuple[dict, dict, list[dict]]:
    ...
    for path in _source_files(root, {".py", ".php", ".js", ".ts", ".json", ".yml", ".yaml"}):
        if "tests" in path.relative_to(root).parts or path.name in {"system.py", "laravel.py"}: continue
        text=_read(path); rel=str(path.relative_to(root))
