...
def file_tree(files: list[dict]) -> str:
    ...
    def branch(directory: dict) -> str:
        rows = []
        for name, value in sorted(directory.items(), key=lambda pair: ("path" in pair[1], pair[0].lower())):
            if "path" not in value:
                rows.append(f"<details class='folder'><summary>▸ {e(name)}</summary>{branch(value)}</details>")
                continue
            symbols = value.get("symbols", [])
            details = "".join(f"<li><b>{e(symbol['name'])}</b> <small>{e(symbol['kind'])} · {e(symbol['source'])}</small>" +
                              ("<ul>" + "".join(f"<li>{e(member['name'])}</li>" for member in symbol.get("members", [])) + "</ul>" if symbol.get("members") else "") + "</li>" for symbol in symbols[:100])
            rows.append(f"<details class='source-file'><summary>{e(name)} <small>{len(symbols)} símbolos · {'grafo' if value['analyzed'] else 'sin nodos del grafo'}</small></summary><p>{e(value['path'])}</p><ul>{details or '<li>Sin símbolos detectados.</li>'}</ul></details>")
        return "".join(rows)
    return branch(tree) or empty("No se encontraron archivos.")
...
def render(context: dict, agent_text: str, graphify_files: list[str]) -> str:
    ...
    onboarding = context.get("agent_onboarding", {})
    onboarding_ready = onboarding.get("status") in {"configured", "unchanged"}
    mcp_ready = onboarding.get("mcp", {}).get("available", False)
    ...
    graph_paths = exploration.get("call_paths", [])
    call_cards = "".join(f"<article class='flow-card'><b>{e(path['name'])}</b><p>{' <em>→</em> '.join(e(step['name']) for step in path['steps'])}</p><small>{e(path['note'])} Ramas adicionales: {path['branches']}.</small></article>" for path in graph_paths)
    recent = "".join(f"<li><strong>{e(change['subject'])}</strong><small>{e(change['date'])} · {e(change['commit'][:10])}</small></li>" for change in system.get("recent_changes", []))
    ...
    views = [
        ...
        section("Archivos y símbolos", f"<div class='tech-hero'><h3>Explorá el proyecto</h3><p>{coverage.get('code_files_with_graph_nodes', 0)} de {coverage.get('eligible_code_files', 0)} archivos de código tienen nodos en el grafo. Estado: {e(coverage.get('status', 'UNKNOWN'))}. Un nodo no garantiza cobertura completa.</p></div><input id='file-search' type='search' placeholder='Buscar archivo o símbolo' aria-label='Buscar archivo o símbolo'><div class='file-tree'>{file_tree(exploration.get('files', []))}</div>", "archivos", "▤"),
        section("Recorridos del grafo", f"<p class='empty'>Estas rutas muestran llamadas conectadas en Graphify. No implican que sean entradas HTTP ni describen todas las ramas.</p><div class='flow-grid'>{call_cards or empty('No hay recorridos de llamadas verificables en el grafo.')}</div>", "recorridos", "⌁"),
        section("Historial Git", f"<article class='detail-card'><h3>Cambios recientes</h3><p>Los mensajes describen cambios registrados; el código actual sigue siendo la fuente para comprender el comportamiento.</p><ul class='detail-list'>{recent or empty('No hay historial disponible.')}</ul></article>", "historial", "◷"),
    ]
...
.file-tree{{max-height:70vh;overflow:auto;margin-top:12px;padding:14px;border:1px solid var(--line);border-radius:9px;background:var(--panel)}}.file-tree details{{padding:3px 4px 3px 14px;border-left:1px solid var(--line)}}.file-tree summary{{cursor:pointer;padding:5px;color:var(--ink)}}.file-tree small{{color:var(--muted)}}.source-file ul{{padding-left:22px}}.source-file li{{padding:3px}}#file-search{{width:100%;padding:11px;margin-top:12px;background:var(--panel);border:1px solid var(--line);border-radius:7px;color:var(--ink)}}
</style></head><body><nav class='top'><div class='brand'><span class='logo'>K</span>klap_context</div><div class='top-title'><b>Comprender el sistema</b><small>Explorá componentes, flujos y evidencia de tu repositorio.</small></div><span class='top-state'>● {state}</span><span class='repo'>{e(project['name'])}</span></nav><div class='shell'><aside class='side'><button class='active' data-go='sistema'><span class='nav-icon'>⌘</span>Sistema</button><button data-go='flujos'><span class='nav-icon'>⌁</span>Flujos</button><button data-go='entradas'><span class='nav-icon'>⇢</span>Entradas</button><button data-go='evidencia'><span class='nav-icon'>◈</span>Evidencias</button><button data-go='archivos'><span class='nav-icon'>▤</span>Archivos</button><button data-go='recorridos'><span class='nav-icon'>⌁</span>Recorridos</button><button data-go='historial'><span class='nav-icon'>◷</span>Git</button><button data-go='tecnico'><span class='nav-icon'>⌬</span>Graphify</button><div class='side-footer'><button data-go='evidencia'><span class='nav-icon'>?</span>Ayuda y confianza</button></div></aside><main>{''.join(views)}</main><aside class='right'><section class='agent-card'><h3>✦ Acciones para agente</h3><p>Generá un briefing listo para usar y profundizá luego en el sistema.</p><button class='copy' onclick='copyContext(this)'>▣ Copiar contexto</button></section><section class='insight-card'><h3>Use this project with your AI agent</h3><ul><li>{'✓' if onboarding_ready else '!'} AGENTS.md {'preparado' if onboarding_ready else 'requiere configuración'}</li><li>✓ Contexto generado en .klap/</li><li>{'✓' if graphify_files else '!'} Grafo Graphify {'disponible' if graphify_files else 'no disponible'}</li><li>{'✓' if mcp_ready else '!'} Configuración MCP {'generada' if mcp_ready else 'pendiente'}</li></ul><p>Comprobá vigencia con <code>klap status</code> y actualizá con <code>klap update</code>.</p></section><section class='insight-card'><h3>💡 Hallazgos principales <span>{min(len(evidence), 9)}</span></h3><ul><li>{len(points)} punto(s) de entrada detectado(s).</li><li>{len(flows)} flujo(s) principal(es) disponible(s).</li><li>{len(evidence)} hecho(s) con respaldo en el código.</li></ul></section><section class='insight-card'><h3>⚡ Siguiente paso</h3><p>Usá las vistas laterales para investigar un área, sin perder el panorama inicial.</p></section></aside></div><script>const agentContext={agent};const views=[...document.querySelectorAll('.view')];const nav=[...document.querySelectorAll('[data-go]')];function go(name){{views.forEach(view=>view.classList.toggle('active',view.dataset.view===name));nav.forEach(button=>button.classList.toggle('active',button.dataset.go===name));history.replaceState(null,'','#'+name);window.scrollTo({{top:0,behavior:'smooth'}})}}nav.forEach(button=>button.addEventListener('click',()=>go(button.dataset.go)));go(location.hash.slice(1)||'sistema');function copyContext(button){{navigator.clipboard.writeText(agentContext).then(()=>{{const old=button.textContent;button.textContent='✓ Contexto copiado';setTimeout(()=>button.textContent=old,1500)}})}}const paths={traces};const search=document.querySelector('#file-search');if(search)search.addEventListener('input',()=>{{const term=search.value.toLocaleLowerCase();document.querySelectorAll('.source-file').forEach(file=>{{file.hidden=!file.textContent.toLocaleLowerCase().includes(term)}});document.querySelectorAll('.folder').forEach(folder=>{{folder.open=!!term}})}});const selector=document.querySelector('#flow-select');if(selector){{const trace=document.querySelector('#flow-trace');const show=()=>trace.textContent=paths[selector.value].join(' → ');selector.addEventListener('change',show);show();}}</script></body></html>"""
